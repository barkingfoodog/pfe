...
when 'create'
  time do
    ...
    schemas = @connection.exec("SELECT schema_name FROM information_schema.schemata WHERE (schema_owner=$1 AND schema_name not in ('contrib', 'dblink', 'information_schema', 'pgtap', 'pgagent', 'pgcrypto', 'pg_catalog') AND schema_name NOT LIKE 'pg_temp%' AND schema_name NOT LIKE 'pg_toast_temp%') OR schema_name='public';", [@user])
